...
<template>
  <div class="flex h-screen bg-[#f5f4f1] dark:bg-[#0c0b18]">
    ...
    <aside
      class="hidden md:flex shrink-0 flex-col border-r border-gray-200 dark:border-gray-800
             bg-white dark:bg-[var(--ui-sidebar-bg)] transition-[width] duration-200 overflow-hidden"
      :class="sidebarCollapsed ? 'w-14' : 'w-56'"
    >
      <!-- Header -->
      <div
        class="h-12 flex items-center shrink-0 border-b border-gray-200 dark:border-gray-800"
        :class="sidebarCollapsed ? 'justify-center px-2' : 'px-4 gap-2'"
      >
        <span
          class="font-semibold text-brand-600 dark:text-brand-400 shrink-0"
          :class="sidebarCollapsed ? 'text-base' : 'text-sm'"
        >
          {{ sidebarCollapsed ? 'P' : 'Papyrus' }}
        </span>
        <span v-if="!sidebarCollapsed" class="text-xs text-gray-400 dark:text-gray-600 uppercase tracking-widest">admin</span>
        <button
          class="ml-auto p-1 rounded text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors shrink-0"
